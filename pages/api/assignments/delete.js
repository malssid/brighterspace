  ...
  if (session) {
    ...
    if (role === 1) {
      const submissions = await query(
        "SELECT * FROM `submissions` WHERE `cid` = ? AND `assnid` = ?",
        [req.body.cid, req.body.assnid]
      );

      if (submissions.length > 0) {
        const clearSubmissions = await query(
          "DELETE FROM `submissions` WHERE `cid` = ? AND `assnid` = ?",
          [req.body.cid, req.body.assnid]
        );
      }

      const result = await query(
        "DELETE FROM `assignments` WHERE `cid` = ? AND `assnid` = ?",
        [req.body.cid, req.body.assnid]
      );
