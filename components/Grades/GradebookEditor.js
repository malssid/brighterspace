    ...
    <Flex direction="column" justify="center" borderRadius="md" m={4} w="full">
      <Box>
        <Heading align="center" size="2xl" mb={4} color="blue.50">
          <Link href={`/course/${props.gradeItemInfo[0].cid}/grades`}>
            Grade Items{" "}
          </Link>
          &gt; {props.gradeItemInfo[0].name}
        </Heading>
      </Box>
      <Box>
        <Table>
          ...
          <Tbody>
            {props.grades &&
              props.grades.map((item, key) => {
                return (
                  <Tr key={key} color="blue.50">
                    <Td>
                      {item.First_name} {item.Last_name} {props.gid}
                    </Td>
                    <Td>
                      <Editable
                        defaultValue={
                          item.grade === null ? "Not assigned" : item.grade
                        }
                        onSubmit={(nextval) =>
                          postUpdatedItem(item.gid, item.pid, {
                            grade: nextval,
                          })
                        }
                      >
